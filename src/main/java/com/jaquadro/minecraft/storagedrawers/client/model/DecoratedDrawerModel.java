...
public abstract class DecoratedDrawerModel implements BakedModel
{
    ...
    public void emitFramedQuads(DrawerModelContext context, Consumer<BakedModel> emitModel) {
        ...
        if (block instanceof BlockStandardDrawersFramed framed) {
            MaterialData matData = context.materialData();
            if (matData != null && !matData.isEmpty()) {
                emitModel.accept(new SpriteReplacementModel(DrawerModelStore.getModel(
                    DrawerModelStore.DynamicPart.FRAMED_DRAWERS_SIDE, dir, half, framed.getDrawerCount()),
                    matData.getEffectiveSide()));

                emitModel.accept(new SpriteReplacementModel(DrawerModelStore.getModel(
                    DrawerModelStore.DynamicPart.FRAMED_DRAWERS_TRIM, dir, half, framed.getDrawerCount()),
                    matData.getEffectiveTrim()));

                emitModel.accept(new SpriteReplacementModel(DrawerModelStore.getModel(
                    DrawerModelStore.DynamicPart.FRAMED_DRAWERS_FRONT, dir, half, framed.getDrawerCount()),
                    matData.getEffectiveFront()));
            }
        }
    }

    public void emitFramedOverlayQuads(DrawerModelContext context, Consumer<BakedModel> emitModel) {
        Direction dir = context.state().getValue(BlockDrawers.FACING);
        boolean half = false;
        Block block = context.state().getBlock();
        if (block instanceof BlockDrawers drawers)
        ...
        if (block instanceof BlockStandardDrawersFramed framed) {
            MaterialData matData = context.materialData();
            if (matData != null && !matData.isEmpty()) {
                emitModel.accept(DrawerModelStore.getModel(
                    DrawerModelStore.DynamicPart.FRAMED_DRAWERS_SHADING, dir, half, framed.getDrawerCount()));
            }
        }
    }
